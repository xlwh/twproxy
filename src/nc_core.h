...
//proxy 实例
struct instance {
    struct context  *ctx;                        /* active context */  //活动的contex，可以有多个contex
    int             log_level;                   /* log level */        //这里定义日志级别
    char            *log_filename;               /* log filename */      //日志文件名字
    char            *conf_filename;              /* configuration filename */   //配置文件路径
    uint16_t        stats_port;                  /* stats monitoring port */
    int             stats_interval;              /* stats aggregation interval */   //统计间隔
    char            *stats_addr;                 /* stats monitoring addr */        //统计地址
    char            hostname[NC_MAXHOSTNAMELEN]; /* hostname */
    size_t          mbuf_chunk_size;             /* mbuf chunk size */
    pid_t           pid;                         /* process id */
    char            *pid_filename;               /* pid filename */           //pid文件
    unsigned        pidfile:1;                   /* pid file created? */     //是否创建pid?
};
